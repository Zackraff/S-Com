...
export default function HeaderView() {
  ...
  return (
    <>
      ...
      <header className={`w-full bg-black text-white px-8 py-6 fixed top-0 left-0 z-20 shadow-md transition-all duration-300 border-b-1 border-black ${scrolled ? "border-b-1 border-gray-600" : ""}`}>
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            <img className="w-8" src="/judol.jpeg" alt="logo" />
      ...
      <aside className={`fixed top-0 right-0 w-64 h-full bg-black text-white p-6 z-30 transform transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"} md:hidden`}>
        <div className="flex align-middle justify-end gap-3 mb-6">
          <button onClick={() => setOpen(!open)} className="md:hidden text-white focus:outline-none text-2xl" aria-label="Toggle Menu">
            {<HiX />}
          </button>
        </div>
